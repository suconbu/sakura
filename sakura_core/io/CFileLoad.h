...
public:
	...
	void ReferTo( const CFileLoad& other, size_t nOffsetBegin, size_t nOffsetEnd );
	//void CopyFrom( const CFileLoad& other );
...
//	LPWSTR	m_pszFileName;	// ファイル名
	...
	//size_t m_nReadBufSize;		// 読み込みバッファ有効サイズ(m_nReadBufOffsetからこのサイズ分が有効)
